...
{ TCustomActionToolBar }
  ...
  protected
    ...
    procedure DoDropCategory(Source: TCategoryDragObject); override;
...
begin
  ...
  begin
    ...
    if Result is TCustomButtonControl then
      with Result as TCustomButtonControl do
        SmallIcon := ActionClient.Items.SmallIcons;
  end;
end;
...
var
  I: Integer;
begin
  // !! Need to find nearest control
  for I := 0 to Source.ActionCount - 1 do
  begin
    ActionClient.Items.Insert(I);
    ActionClient.Items[I].Action := Source.Actions[I];
  end;
...
begin
  ...
  if (ItemCount = 0) or not AutoSize then exit;
  FirstItem := FindFirst;
  LastItem := FindLast;
      ...
      case Orientation of
        boLeftToRight:
          if LastItem = AnItem.ActionClient then
            Result := AnItem.Left + AnItem.Width >= ScrollBtnBounds.Left
          else
            Result := LastItem.Control.Left + LastItem.Control.Width + AnItem.Width >
        ...
        boRightToLeft:
          if LastItem = AnItem.ActionClient then
            ...
            Result := AnItem.Width > LastItem.Control.Left -
        ...
        boBottomToTop:
          if LastItem = AnItem.ActionClient then
            ...
            Result := AnItem.Height > LastItem.Control.Top -
              (ScrollBtnBounds.Top + (ScrollBtnBounds.Bottom - ScrollBtnBounds.Top));
        boTopToBottom:
          if LastItem = AnItem.ActionClient then
            Result := AnItem.Top + AnItem.Height >= ScrollBtnBounds.Top
          else
            Result := LastItem.Control.Top + LastItem.Control.Height + AnItem.Height >
              ClientHeight - VertMargin - (ScrollBtnBounds.Bottom - ScrollBtnBounds.Top);
      end;
  end;
...
begin
  if AutoSize or (FHideLevel > 0) then exit;
  if Assigned(FScrollBtn) then
  ...
  begin
    ...
    if Assigned(FHiddenItems) and (FHiddenItems.Count > 0) then
      ...
      begin
        ...
        if FHiddenItems.Count = 0 then
          FreeAndNil(FHiddenItems);
      end;
  if not Assigned(ActionManager.DefaultActionBars) and Assigned(FScrollBtn) then
    FScrollBtn.Enabled := Assigned(FHiddenItems) and not (csDesigning in ComponentState);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      Parent := Self;
      ActionClient := AC;
      with Self.ClientToScreen(Point(Self.FScrollBtn.Left - FPopupBar.Width +
         Self.FScrollBtn.Width, Self.FScrollBtn.Top + Self.FScrollBtn.Height)) do
      begin
        Expand(True);
        Popup(X, Y);
      end;
...
procedure TCustomActionToolBar.SetActionClient(const Value: TActionClient);
begin
  inherited SetActionClient(Value);
...
procedure TCustomActionToolBar.SetAutoSize(Value: Boolean);
begin
