...
{$ENDIF}
    ...
    function GetQueryFromType: string; {$IFDEF MSWINDOWS}virtual;{$ENDIF}
      ...
      try
        ...
        begin
          ...
          if Params[I].ParamType <> ptOutput then
            ...
            FillChar(RecBuffer^, DataLen, 0);
          if iFldType in [fldBlob, fldZString, fldBYTES, fldVARBYTES] then
          ...
          case iFldType of
            fldZString:
              begin
                if iSubType <> fldstFIXED then
                  ArgDesc.iUnits1 := DataLen - 1; {Do not include null terminator}
                ArgDesc.iUnits2 := 0;
              end;
            fldBYTES:
              ARgDesc.iUnits1 := DataLen;
            fldVARBYTES:
              ArgDesc.iUnits1 := DataLen - 2;
            fldFLOAT:
              begin
                ArgDesc.iUnits1 := SizeOf(Double);
                if (ArgDesc.iLen <> 8) and (ArgDesc.iLen <> 4 ) then
                  ArgDesc.iLen := Sizeof(Double);
              end;  
            fldFMTBCD, fldBCD:
              iFldType := fldBCD;   { DBExpress does not distinguish }
        ...
        Status := Command.setParameter(iFldNum - ChildPosArray[I], ChildPosArray[I], TSTMTParamType(ArgDesc.iArgType),
                iFldType, iSubType, Integer(ArgDesc.iUnits1),
                Integer(ArgDesc.iUnits2), ArgDesc.iLen, RecBuffer, IInd);
        if (Status <> SQL_Success) then
          Sender.SQLError(Status, exceptConnection);
      finally
...
begin
  ...
  try
    ...
    if trim(LoginParams.Values[ szUSERNAME ]) <> '' then
      FSQLMetaData.SetOption(eMetaSchemaName, LongInt(trim(LoginParams.Values[ szUSERNAME ])));
    ConnectionOptions;
...
begin
  ...
  try
    ...
    begin
      DS.CheckStatement;
      Check(DS.FSQLCommand.prepare(PChar(SQLText), ParamCount));
...
begin
  if (FMaxColSize > 0) then
    FFieldBuffer := AllocMem(FMaxColSize);
...
begin
  ...
  begin
    if Field.Size > Field.DataSize then
...
begin
  ...
  begin
    ...
    if (Params[I].ParamType in [ptOutput, ptResult, ptInputOutput]) and
       (ArgDesc.iDataType <> ftCursor) then
    begin
      RecBuffer := AllocMem(ArgDesc.iLen);
      try
        Check(FSQLCommand.getParameter(i+1, 0, RecBuffer, ArgDesc.iLen, IsNull));
        if IsNull = 1 then
          Params[I].Value := Null
        else
          Params[I].SetData(RecBuffer);
  ...
  case FSchemaInfo.FType of
    ...
    stProcedures:
      Status := GetInternalConnection.FSQLMetaData.getProcedures(
                  WildCard, (eSQLProcedure or eSQLPackage), FSQLCursor);
    stProcedureParams:
