...
const
  ...
  { From DB.pas }
  MaxFMTBcdFractionSize = 64;  // max supported by Midas
  MaxFMTBcdDigits =   32;  // max supported by Midas
...
{ TFMTBcdVariantType }
  ...
  public
    ...
    procedure CastTo(var Dest: TVarData; const Source: TVarData; const VarType: TVarType); override;
...
{ TFMTBcdData }
  ...
  public
    // The many ways to create a variant
...
const
  cBooleanArray: array [Boolean] of Integer = (0, -1);
var
  vTemp: TVarData;
begin
  VarDataInit(vTemp);
  VarDataCopyNoInd(vTemp, Source);
  try
    {$RANGECHECKS ON}
    case vTemp.VType of
      varSmallInt:
        TFMTBcdVarData(Dest).VBcd := TFMTBcdData.Create(vTemp.VSmallInt);
      varInteger:
        TFMTBcdVarData(Dest).VBcd := TFMTBcdData.Create(vTemp.VInteger);
      varSingle:
        TFMTBcdVarData(Dest).VBcd := TFMTBcdData.Create;
      varDouble:
        TFMTBcdVarData(Dest).VBcd := TFMTBcdData.Create(vTemp.VDouble, 32, 8);
      varCurrency:
        TFMTBcdVarData(Dest).VBcd := TFMTBcdData.Create(vTemp.VCurrency);
      varOleStr:
        TFMTBcdVarData(Dest).VBcd := TFMTBcdData.Create(vTemp.VOleStr, 32, 8);
      varString:
        TFMTBcdVarData(Dest).VBcd := TFMTBcdData.Create(string(vTemp.VString^), 32, 8);
      varNull:
        TFMTBcdVarData(Dest).VBcd := TFMTBcdData.Create(NullBcd);
    else
      RaiseCastError;
    end;
    {$RANGECHECKS OFF}
...
procedure TFMTBcdVariantType.CastTo(var Dest: TVarData;
  const Source: TVarData; const VarType: TVarType);
var
  vTemp: TVarData;
begin
  VarDataInit(vTemp);
  ...
  try
    ...
    case VarType of
      varSmallInt:
        Dest.VSmallInt := SmallInt(BcdToInteger(TFMTBcdVarData(vTemp).VBcd.Bcd));
      varInteger:
        Dest.VInteger := BcdToInteger(TFMTBcdVarData(vTemp).VBcd.Bcd);
      varSingle:
        Dest.VSingle := BcdToDouble(TFMTBcdVarData(vTemp).VBcd.Bcd);
      varDouble:
        Dest.VDouble := BcdToDouble(TFMTBcdVarData(vTemp).VBcd.Bcd);
      varCurrency:
        Dest.VCurrency := BcdToDouble(TFMTBcdVarData(vTemp).VBcd.Bcd);
      varOleStr:
        Dest.VOleStr := StringToOleStr(TFMTBcdVarData(vTemp).VBcd.AsString);
      varString:
        string(Dest.VString) := TFMTBcdVarData(vTemp).VBcd.AsString;
    else
      RaiseCastError;
    end;
    {$RANGECHECKS OFF}
...
function TFMTBcdVariantType.LeftPromotion(const V: TVarData;
  const Operator: TVarOp; out RequiredVarType: TVarType): Boolean;
begin
  RequiredVarType := VarType; // fine but convert it to FMTBcd
  case V.VType of
       varNull, varSmallInt, varInteger, varSingle, varDouble, varCurrency,
       varOleStr, varBoolean, varByte, varString:
    Result := True;
  else
    Result := False;
  end;
  if V.VType = VarType then Result := True;
end;

function TFMTBcdVariantType.RightPromotion(const V: TVarData;
  const Operator: TVarOp; out RequiredVarType: TVarType): Boolean;
begin
  RequiredVarType := VarType; // fine but convert it to FMTBcd
  case V.VType of
...
(*function VarFMTBcdCreate(const AValue: Integer; Precision: Word = 10; Scale: Word = 0): Variant; overload;
begin
  VarClear(Result);
  TFMTBcdVarData(Result).VType := FMTBcdVariantType.VarType;
  TFMTBcdVarData(Result).VBcd := TFMTBcdData.Create(AValue);
end;*)
...
//  if Value[1] = '-' then Inc(DecPos);
  ...
  begin
    ...
    P := PP+1;
    if Byte(P^) >= 53  then // if '5' or more
      PP^ := Char(Byte(PP^)+1);
...
begin
  ...
  begin
    ...
    Wid := LB;
    Len := LA;
  end;  // to get rid of GetDecimalPosition
...
begin
  ...
  while P^ <> #0 do
  begin
    if not (P^ in ['0'..'9', '-', Dot, Comma]) then
    begin
      Result := True;
      break;
    end;
...
asm
        ...
        CLD
@@1:    LODSB                     // strip leading 0's from input1
        CMP     AL,0
        JNE     @@2
        SUB     CX,2
        JMP     @@1
@@2:    MOV     AH,[EDI]          // strip leading 0's from input1
        INC     EDI
        CMP     AH,0
        JE      @@2
        JMP     @@4
@@3:    CMP     CX,0
        ...
        INC     EDI               // advance EDI
@@4:    CMP     AL,ah             // start by comparing 2 nibbles at once
        JNE     @@5
        SUB     CX,1
        CMP     CX,0
        JE      @@9
        SUB     CX,1
        JMP     @@3
@@5:    MOV     BL,AH             // don't appear to match:
...
@@6:    CMP     BH,AH             // we know they're not the same
        JA      @@8
@@7:    MOV     Result,1          // input1 > input2
        JMP     @@9
@@8:    MOV     Result,-1         // input2 > input1
@@9:    POP     EBX
        POP     EDI
        POP     ESI
end;
...
var
  Prec, Scale1, Scale2, Excess: Word;
  IntDigitsUsed, Req, ReqDecDigits: Word;
begin
  Req := Max( bcdIn1.Precision - (bcdIn1.SignSpecialPlaces and 63),
               bcdIn2.Precision - (bcdIn2.SignSpecialPlaces and 63));
  ReqDecDigits := Max( (bcdIn1.SignSpecialPlaces and 63),
               (bcdIn2.SignSpecialPlaces and 63));
  Prec := ExtraDigits + Req + ReqDecDigits;
  if Prec > MaxFMTBcdFractionSize then
  begin
    Excess := Prec - (MAXFMTBcdFractionSize div 2);
    Prec := (MAXFMTBcdFractionSize div 2);
    IntDigitsUsed := ExtraDigits + Max(SignificantIntDigits(PChar(@BcdIn1.Fraction), SmallInt(BcdIn1.Precision - (BcdIn1.SignSpecialPlaces and 63))),
                  SignificantIntDigits(PChar(@BcdIn2.Fraction), SmallInt(BcdIn2.Precision - (BcdIn2.SignSpecialPlaces and 63))));
    if Excess > ((MAXFMTBcdFractionSize div 2) + IntDigitsUsed) then
      ReqDecDigits := Max(0, ReqDecDigits-Excess);
  end;
  if (bcdIn1.SignSpecialPlaces and (1 shl 7)) <> 0 then
    Scale1 := Word((ReqDecDigits and 63) or Word(1 shl 7))
  else
    Scale1 := ReqDecDigits;
  if (bcdIn2.SignSpecialPlaces and (1 shl 7)) <> 0 then
    Scale2 := Word((ReqDecDigits and 63) or Word(1 shl 7))
  else
    Scale2 := ReqDecDigits;
  NormalizeBcd(bcdIn1, bcdOut1, Prec, Scale1);
  NormalizeBcd(bcdIn2, bcdOut2, Prec, Scale2);
end;
...
begin
  ...
  begin  // both Negative or both Positive
    ...
    begin
      ...
      pBcd1 := pCHAR(@TempBcd1.Fraction);   // move to fractions
      pBcd2 := pCHAR(@TempBcd2.Fraction);   // move to fractions
      Result := CompareNormalizedFractions(pBcd1, pBcd2, Bcd1.Precision, Bcd2.Precision);
    end;
...
function FractionToStr(const pIn: PChar; count: SmallInt;
         DecPosition: ShortInt; Negative: Boolean): string;
var
  ...
  begin
    if (DecPosition > 0) and (NibblesIn  = DecPosition) then
    begin
