...
type
 ...
 protected
  ...
  private
    FID: Integer;
  public
    constructor CreateID(const AMsg: string; AID: Integer);
...
begin
  if Assigned(OnGetDisplayText) then
    ...
    if VarIsEmpty(GetValue) or VarIsNull(GetValue) then
      Result := ''
    else
      Result := GetValue;
end;
  ...
  begin
    ...
    if IsMultiValueField then
      ...
      begin
        ...
        try
          if ConvertValue(FieldValue) <> GetValue then
            Result := True;
        except
          Result := True;
        end;
...
begin
  raise EAdapterFieldException.Create(sBlankPassword, sLoginPassword);
end;

procedure TCustomLoginFormAdapter.RaiseBlankUserName;
begin
  raise EAdapterFieldException.Create(sBlankUserName, sLoginUserName);
end;
...
begin
  ...
  begin
    ...
    begin
      try
        if FStrings.Values[Result] = Value then
          Exit;
      except
...
begin
  ...
  if Assigned(OnUpdateValue) then
    with GetActionFieldValues(AActionRequest) do
      OnUpdateValue(Self, Values[AFieldIndex]);
end;
...
begin
  Result := Adapter.RecordCount >
      Adapter.PageSize * (Adapter.CurrentPage + 1);
  if Result then
