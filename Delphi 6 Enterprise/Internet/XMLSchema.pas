...
begin
  RegisterChildNodes([SRestriction],
    [TXMLTypeRestriction]);
  inherited;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FTypeDef := SchemaDef.ComplexTypes.Find(TypeName);
        if not Assigned(FTypeDef) then
        begin
  ...
  begin
    for I := 0 to SchemaDocRefs.Count - 1 do
      if Assigned(SchemaDocRefs[I].SchemaRef) and
         SameNamespace(SchemaDocRefs[I].SchemaRef.TargetNamespace, ItemNS) then
      begin
        SchemaObj := (SchemaDocRefs[I].SchemaRef as IXMLNodeAccess);
        ImportCollection := SchemaObj.FindHostedNode(FCollectionClassType) as IXMLSchemaItems;
  ...
  begin
    for I := 0 to ParentNode.ChildNodes.Count - 1 do
      if Supports(ParentNode.ChildNodes[I], IXMLSchemaDef, SchemaDef) and
        SameNamespace(SchemaDef.TargetNamespace, ItemNS) then
      begin
...
begin
  ...
  begin
    ItemNS := FindNamespaceURI(Name);
    if (ItemNS <> '') and not SameNamespace(SchemaDef.DOMNode, ItemNS) and
      not SameNamespace(SchemaDef.TargetNamespace, ItemNS) then
    begin
      Result := SearchLinkedSchemas(SchemaDef.SchemaImports, ItemNS);
      if not Assigned(Result) then
      begin
        Result := SearchLinkedSchemas(SchemaDef.SchemaIncludes, ItemNS);
        if not Assigned(Result) and (SchemaDef.ParentNode.NodeType = ntElement) then
          Result := SearchSiblingSchemas(SchemaDef.ParentNode, ItemNS);
      end;
    end;
  end
end;
...
begin
  for Result := 0 to Count - 1 do
    if (List[Result] as IXMLSchemaItem).Name = Name then Exit;
  Result := -1;
end;
...
var
  LocalName: DOMString;
begin
  if not IsPrefixed(Name) or
     (SchemaDef.FindNamespaceURI(Name) = SchemaDef.NamespaceURI) then
  begin
    LocalName := ExtractLocalName(Name);
