...
uses Variants, ComObj, msxml, xmldom, Classes;
...
begin
  for I := Low(GuidList) to High(GuidList) do
    if CoCreateInstance(GuidList[I], nil, CLSCTX_INPROC_SERVER or
      CLSCTX_LOCAL_SERVER, IDispatch, Result) = S_OK then Exit;
end;

function CreateDOMDocument: IXMLDOMDocument;
begin
  Result := TryObjectCreate([CLASS_DOMDocument30, CLASS_DOMDocument26,
...
begin
  Result := MSNode.baseName; { name difference }
end;
...
begin
  DOMVendorNotSupported('setNamedItemNS', SMSXML); { Do not localize }
end;
...
begin
  Result := IDOMCDATASection(TMSDOMCDATASection.Create(MSDocument.createCDATASection(data)));
end;

function TMSDOMDocument.createComment(const data: DOMString): IDOMComment;
begin
  Result := IDOMComment(TMSDOMComment.Create(MSDocument.createComment(data)));
end;

function TMSDOMDocument.createDocumentFragment: IDOMDocumentFragment;
begin
  Result := IDOMDocumentFragment(TMSDOMDocumentFragment.Create(MSDocument.createDocumentFragment));
end;
