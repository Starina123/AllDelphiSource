...
begin
  if Assigned(Attributes) then
    case Attributes.LabelPosition of
      ...
      lposAbove:
        Result :=
          StartFields(1) + Format('%0:s>%1:s</td></tr><tr><td ' + sColSpanAttr + '="2" %2:s>%3:s</td>'#13#10,
            [Attributes.LabelAttributes, HTMLLabel, Attributes.ControlAttributes, HTMLField]);
      lposBelow:
        Result :=
          StartFields(1) + Format('%0:s>%1:s</td></tr><tr><td ' + sColSpanAttr + '="2" %2:s>%3:s</td>'#13#10,
...
function TFormLayout.NextColumn(SubItemCount: Integer): string;
  ...
  begin
    Result := Format('<td ' + sColSpanAttr + '="%d"', [FColumnCount * 2]);
  end;
  ...
  begin
    ...
    if SubItemCount = 1 then
      Span := 2;
    if Span <> -1 then
      Result := Format('<td ' + sColSpanAttr + '="%d"', [Span])
    else
      Result := '<td';
  end;
begin
...
begin
  ...
  begin
    ...
    begin
      if Assigned(Dispatcher) and Assigned(Dispatcher.Request) then
        Redirect := Format('http://%s%s', [Dispatcher.Request.Host, Dispatcher.Request.InternalScriptName]);
