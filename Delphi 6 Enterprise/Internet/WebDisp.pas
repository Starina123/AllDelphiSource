...
type
  ...
  private
    ...
    procedure ExtractSession(ASessionsService: ISessionsService;
      ARequest: TWebRequest);
    constructor Create(ASessions: ISessionsService; AWebSession: IWebSession); overload;
    procedure UpdateSessionCookie(AResponse: TWebResponse; const AValue: string);
  protected
...
begin
  ...
  for I := 0 to ParamValues.Count - 1 do
    AddNameValue(ParamValues.Names[I], ExtractStringValue(ParamValues[I]));
end;
...
begin
  ...
  begin
    SourceLine := Copy(EScriptError(E).Errors[I].SourceLine, 1, EScriptError(E).Errors[I].CharPos);
    SourceLine := StringReplace(SourceLine, '<', '&lt;', [rfReplaceAll]);
    SourceLine := StringReplace(SourceLine, '>', '&gt;', [rfReplaceAll]);
    SourceLine := SourceLine + '<font color="#FF0000">|</font>' +
        StringReplace(
...
begin
  ...
  if Assigned(FSessions) then
    ...
    begin
      ExtractSession(Intf, ARequest);
      UpdateSessionCookie(AResponse, '');
    end;
  inherited Create;
end;
...
const
  sSessionCookieName = 'WebBrokerSessionID';

procedure TWebSession.ExtractSession(ASessionsService: ISessionsService; ARequest: TWebRequest);
var
  Intf: ISessionsServiceConnectID;
  ID: TSessionID;
begin
  ...
  begin
    if ARequest.CookieFields.Values[sSessionCookieName] <> '' then
      try
        ID := ARequest.CookieFields.Values[sSessionCookieName];
...
procedure TWebSession.UpdateSessionCookie(AResponse: TWebResponse; const AValue: string);
var
  I: Integer;
begin
    ...
    begin
      ...
      begin
        if Supports(FWebSession, IWebSessionID, GetID) then
          UpdateSessionCookie(AResponse, GetID.ID)
        else
          UpdateSessionCookie(AResponse, GetID.ID);
      end
      else
