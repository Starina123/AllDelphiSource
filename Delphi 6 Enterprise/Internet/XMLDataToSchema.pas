  ...
  begin
    with ComplexTypeDef do
      ...
      begin
        ...
        begin
          ElementDef := ElementDefs.Find(ChildNode.LocalName);
          ...
          begin
            ...
            ElementDefs.Add(ChildNode.LocalName, ChildType);
          end else
          begin
    ...
    with ComplexTypeDef do
      ...
      begin
        ...
        if Pos('xml', attributeNode.Prefix) <> 1 then
          if AttributeDefs.IndexOfItem(AttributeNode.LocalName) = -1 then
            AttributeDefs.Add(AttributeNode.LocalName, GetDataType(AttributeNode));
      end;
  end;
...
begin
  Result := MakeTypeName(Node.LocalName);
...
begin
  if not Node.IsTextElement then
    ...
    begin
      ChildNode := Node.ChildNodes[I];
      ElementDef := ExistingType.ElementDefs.Find(ChildNode.LocalName);
      ...
      begin
        if IsComplexType(ChildNode) then
          ChildType := AddComplexType(ChildNode) else
          ChildType := GetDataType(ChildNode);
        ExistingType.ElementDefs.Add(ChildNode.LocalName, ChildType);
      end else
      begin
  ...
  begin
    ...
    AttrDef := ExistingType.AttributeDefs.Find(AttrNode.LocalName);
    if not Assigned(AttrDef) then
      ExistingType.AttributeDefs.Add(AttrNode.LocalName, GetDataType(AttrNode));
  end;
...
begin
  ...
  begin
    ...
    begin
      SchemaDef.ElementDefs.Add(XMLDoc.DocumentElement.LocalName,
