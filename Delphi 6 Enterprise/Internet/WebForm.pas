...
begin
  if AVarName <> '' then
    Result := Format('%s.InputName', [AVarName])
  else
    Result := '';
end;
...
begin
  ...
  if Supports(ParentLayout, ILayoutWebContent, Intf) then
    ...
    begin
      Result := Result + ChildTableIndent(GetParentComponent) + '</table>'#13#10;
...
const
  ...
  sWriteHiddenFields =
  '<%%%2:s if (%0:s.%1:s != null)'#13#10 +
   '%2:s   {'#13#10 +
   '%2:s     %0:s.%1:s.WriteFields(Response)'#13#10 +
   '%2:s   } %%>'#13#10;
  sHiddenFields = 'HiddenFields';
  sHiddenRecordFields = 'HiddenRecordFields';
  // Params 0 - AdapterName, 1 - Row
  sJSGridForEach =
  '<%% {'#13#10 +
   '   var ri = 0'#13#10 +
   '   var e = new Enumerator(%0:s.Records)'#13#10 +
...
function TCustomAdapterGrid.FormatTable(Layout: TLayoutWebContent;
  ...
  begin
    ...
    Result := Format('<table%s>'#13#10, [Attribs]);
  end;
  ...
  begin
    ...
    Result := FieldIndent(Self) + Format('<tr%s>'#13#10, [Attribs]);
  end;
...
begin
  ...
  Result := Result + FieldIndent(Self) + '</tr>'#13#10;

  VarName := MakeAdapterVariableName(Self);
  OneRow := OneRow + FieldIndent(Self) + '</tr>'#13#10 + FormatHiddenRecordFields(VarName);
  if VarName <> '' then
...
const
  sWriteColTextFunctionName = 'WriteColText';
  sWriteColTextFunctionParameters = '(%s.DisplayText)';
  sWriteColTextFunction =
  'function %s(t)'#13#10 +
  '{'#13#10 +
  '   Response.Write((t!="")?t:"&nbsp")'#13#10 +
  '}'#13#10;
...
begin
  ...
  if Attrs <> '' then
    Result := Format('<span%s>%s</span>', [Attrs, Result]);
end;
...
begin
  ...
  Result := Result + Format('<ul %0:s>'#13#10'%1:s</ul>',
                   [Attrs,
                   '<%=obj.text%>']);
end;
...
begin
  AddQuotedAttrib(Attrs, sStyleAttr, Style);
  AddQuotedAttrib(Attrs, sClassAttr, StyleRule);
  AddIntAttrib(Attrs, 'width', ImageWidth);
  AddIntAttrib(Attrs, 'height', ImageHeight);
  AddCustomAttrib(Attrs, Custom);
  HREF := Format('<%%=(%0:s.Image!=null)?%0:s.Image.AsHREF:''''%%>', [VarName]);
  Result := Format('<img src="%0:s"%2:s alt="<%%=%1:s.DisplayText%%>">', [HREF, VarName,
    Attrs]);
end;
...
begin
  ...
  if FBreakButtons then
    Result := Format('%0:s%3:s>%1:s</td>'#13#10, [Result, HTMLButton, FieldIndent, Attributes.ControlAttributes])
  else if FButtonIndex = 1 then
    Result := Format('%0:s%3:s>%1:s</td>'#13#10, [Result, HTMLButton, FieldIndent, Attributes.ControlAttributes])
  else
    Result := Format('%0:s%2:s<td%3:s>%1:s</td>'#13#10,
...
begin
  if Assigned(Attributes) then
    Result := StartFields(1) + Format('%0:s>%1:s</td>', [Attributes.ControlAttributes, HTMLField])
  else
    Result := StartFields(1) + Format('>%0:s</td>', [HTMLField]);
end;
    ...
    case Attributes.LabelPosition of
      ...
      begin
        Result := StartFields(2) +
           Format('%0:s>%1:s</td>'#13#10 + FieldIndent + '<td %2:s>%3:s</td>'#13#10,
            [Attributes.LabelAttributes, HTMLLabel, Attributes.ControlAttributes, HTMLField]);
      end;
      lposRight:
      begin
        Result := StartFields(2) +
          Format('%0:s>%1:s</td>'#13#10'<td %2:s>%3:s</td>'#13#10,
            [Attributes.ControlAttributes, HTMLField, Attributes.LabelAttributes, HTMLLabel]);
      end;
      lposAbove:
        Result :=
          StartFields(1) +
           Format('%0:s>%1:s</td></tr>'#13#10'<tr><td ' + sColSpanAttr + '="2" %2:s>%3:s</td>'#13#10,
            [Attributes.LabelAttributes, HTMLLabel, Attributes.ControlAttributes, HTMLField]);
      lposBelow:
        Result :=
          StartFields(1) +
...
begin
  if Assigned(Attributes) then
    Result := StartFields(1) + Format('%0:s>'#13#10 + '%1:s' + FieldIndent + '</td>'#13#10, [Attributes.ControlAttributes, HTMLTable])
  else
    Result := StartFields(1) + Format('>'#13#10 + '%0:s' + FieldIndent + '</td>'#13#10, [HTMLTable]);
...
begin
  ...
  begin
    ...
    if not FInRow then
      Result := Result + StartRow + FieldIndent + '<td'
    else
      Result := FieldIndent + '<td';
    Inc(FColumnIndex);
  end;
...
begin
  if FInTable then
  begin
    Result := EndRow + TableIndent + '</table>'#13#10;
    Dec(FTableLevel);
    FInTable := False;
  end
  else
...
function TCustomAdapterForm.FormatForm(ParentLayout: TLayout;
  ...
  begin
    Result := Format('<input ' + sTypeAttr + '="hidden" ' + sNameAttr + '="%s">'#13#10, [sActionRequest]);
  end;
...
begin
  ...
  { Value is not encoded. So embedded quotes, '>', '<', may not work properly with some HTML elements }
  Result := Format('<input ' + sTypeAttr + '="text"%0:s ' + sNameAttr + '="<%%=%2:s%%>" ' + sValueAttr + '="%1:s" >',
...
begin
  ...
  AddIntAttrib(Attrs, sSizeAttr, DisplayWidth);
  { Value is not encoded. So embedded quotes, '>', '<', may not work properly with some HTML elements }
  Result := Format('<input ' + sTypeAttr + '="file"%0:s ' + sNameAttr + '="<%%=%1:s%%>"',
                   [Attrs,
                   HTMLName]);
end;
...
begin
  ...
  { Value is not encoded. So embedded quotes, '>', '<', may not work properly with some HTML elements }
  Result := Format('<textarea%0:s ' + sNameAttr + '="<%%=%2:s%%>">'#13#10'%1:s'#13#10 + FieldIndent(Self) + '</textarea>',
                   [Attrs, Format(sFieldText, [VarName]),
                   HTMLName]);
end;
...
begin
  ...
  AddIntAttrib(Attrs, sMaxLengthAttr, MaxLength);
  { Value is not encoded. So embedded quotes, '>', '<', may not work properly with some HTML elements }
  Result := Format('<input ' + sTypeAttr + '="password"%0:s ' + sNameAttr + '="<%%=%2:s%%>" ' + sValueAttr + '="%1:s" >',
                   [Attrs, Format(sFieldText, [VarName]),
                   HTMLName])
end;
...
begin
  Result := Format('<%%'#13#10 + sInputGroupFunction + '%%>', [sRadioGroupFunctionName, 'radio']);
end;

function CheckBoxGroupFunction(AComponent: TComponent): string;
begin
  Result := Format('<%%'#13#10 + sInputGroupFunction + '%%>', [sCheckBoxGroupFunctionName, 'checkbox']);
end;
...
begin
  ...
  if SelectRows <> -1 then
    ...
    SizeAttr := '<%%=obj.count%%>';
  Result := Result + Format('<select ' + sSizeAttr + '="' + SizeAttr + '" ' + sNameAttr + '="<%%=%1:s%%>"%0:s>%2:s</select>',
                   [Attrs,
                   HTMLName, '<%=obj.text%>']);
end;
...
var
  ...
  begin
    ...
    if Trim(FErrorPageName) = '' then
      LinkToErrorPage := 'Page.Name'
    else
      LinkToErrorPage := Format('"%s"', [FErrorPageName]);
    Result :=
       Format('<%%=%0:s.LinkToPage(%1:s, %2:s).AsFieldValue%%>', [VarName,
  ...
  begin
    if not (coNoScript in Options.Flags) then
      Result := Format('%0:s.%1:s.value=''%2:s'';%0:s.submit();return false;', [
  ...
  begin
    ...
    // 3 - Indent
    Result := Format(
  '<%% if (%0:s.Array != null)'#13#10 +
   '   {'#13#10 +
   '     var c=0'#13#10 +
   '     var cols=%1:d %%>'#13#10 +
   '%3:s<table><tr>'#13#10 +
  '<%%   var e = new Enumerator(%0:s.Array)'#13#10 +
   '     for (; !e.atEnd(); e.moveNext())'#13#10 +
   '     {'#13#10 +
   '       if (cols > 0 && c %% cols == 0 && c != 0)'#13#10 +
   '       { %%>'#13#10 +
   '%3:s  </tr><tr>'#13#10 +
  '<%%     } %%>'#13#10 +
   '%3:s  <td>'#13#10 +
  ...
  '<%%     c++'#13#10 +
   ...
   '     e.moveFirst() %%>'#13#10 +
   '%3:s</tr></table>'#13#10 +
  '<%% } %%>'#13#10, [VarName, DisplayColumns, HTMLElement, Indent]);
  end;
  ...
  '<%%   {'#13#10 +
   '     var Image'#13#10 +
   '     Image = %0:s.GetEventImage(%1:s,''%3:s'', %4:s)'#13#10 +
   '     if (Image != null)'#13#10 +
   '     { %%>'#13#10 +
   '%5:s%3:s="if (document.images) document.images.%2:s.src=''<%%= Image.AsHREF %%>''"'#13#10 +
  '<%%   }'#13#10 +
   '     } %%>'#13#10, [ProducerVarName, VarName, ImageName, Event, Text, Indent]);
  end;

  function FormatImageEvents(const ProducerVarName, VarName, ImageName, Text, Indent: string): string;
  begin
    Result := ' ';
    Result := Result + FormatImageEvent('onmousedown', VarName, ImageName, ProducerVarName, Text, Indent);
    Result := Result + FormatImageEvent('onmouseover', VarName, ImageName, ProducerVarName, Text, Indent);
    Result := Result + FormatImageEvent('onmouseout', VarName, ImageName, ProducerVarName, Text, Indent);
    if Result = ' ' then
      Result := '';
  end;
  ...
  begin
    ...
    begin
      ...
      if IsList then
        Text := Format('%0:s.DisplayLabel', [VarName])
      else
        Text := Format('''%s''', [Self.Caption]);
      HREF := Format('<%%=(%1:s.GetImage(%0:s, %2:s)!=null)?%1:s.GetImage(%0:s, %2:s).AsHREF:''''%%>', [VarName, ProducerVarName,
        Text]);
      if UseEvents then
      begin
        ...
        AddQuotedAttrib(Attrs, 'name', ImageName);
      end;
    end;
    if IsList then
      Text := Format('<%%%0:s.DisplayLabel%%>', [VarName])
      ...
      Text := Self.Caption;
    Result := Format('<img border="0" src="%0:s"%1:s alt="%2:s"/>', [HREF, Attrs, Text, ImageName]);
    Result := Format(#13#10 + sIfEnabledElse,
      [VarName,
      Format('<a %3:shref="%2:s" onclick="%0:s">%1:s</a>', [AnchorClickEvent(VarName), Result, FormAction,
        Events]),
      Format('<a>%0:s</a>', [Result]),
  ...
  begin
    ...
    Result := Format(#13#10 + sIfEnabledElse,
      [VarName,
      Format('<a href="%2:s"%0:s onclick="%3:s">%1:s</a>', [Attrs, Text, FormAction,
        AnchorClickEvent(VarName)]),
      Format('<a%0:s>%1:s</a>', [Attrs, Text]),
      ChildTableIndent(Self)]);
    if IsList then
      Result := #13#10 + FormatList(VarName, Result, ChildTableIndent(Self));
...
begin
  ...
  try
    ...
    AddQuotedAttrib(Attribs, sClassAttr, StyleRule);
    AddCustomAttrib(Attribs, Custom);
    FFormLayout.FTableHeader := Format('<table%s>', [Attribs]);
    Result := '';
...
begin
  if Assigned(FAdapter) then
    Result := Format(
     #13#10 +
    '<%% {'#13#10 +
     '   var e = new Enumerator(%0:s.Errors)'#13#10 +
     '   for (; !e.atEnd(); e.moveNext())'#13#10 +
     '   {'#13#10 +
     '     Response.Write("<li>" + e.item().Message)'#13#10 +
     '   }'#13#10 +
     '   e.moveFirst()'#13#10 +
     '   } %%>'#13#10 + ChildTableIndent(Self),
     [FullyQualifiedFieldName(FAdapter)])
  else
    Result := '';
  if Result <> '' then
...
begin
  ...
  try
    ...
    FormLayout.TableHeader :=
      Format('<table%s>', [GetLayoutAttributes.ControlAttributes]);
    for I := 0 to VisibleCommands.Count - 1 do
    begin
...
begin
  AddQuotedAttrib(Attribs, sAlignAttr, HTMLAlignValues[Align]);
  AddQuotedAttrib(Attribs, sVAlignAttr, HTMLVAlignValues[VAlign]);
  Result := Format(FieldIndent(Self) + '<td%0:s><div>'#13#10'%1:s%2:s</div></td>'#13#10, [Attribs,
...
begin
  ...
  AddQuotedAttrib(Attribs, sVAlignAttr, HTMLVAlignValues[Self.CaptionAttributes.VAlign]);
  AddQuotedAttrib(Attribs, sAlignAttr, HTMLAlignValues[Self.CaptionAttributes.Align]);
  Result := Format(FieldIndent(Self) + '<th%0:s>%1:s</th>'#13#10, [Attribs, Caption]);
end;
