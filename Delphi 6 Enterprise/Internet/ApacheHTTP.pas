    ...
    25: begin
          ...
          begin
            ...
            begin
              ...
              GetMem(buf, size + 1);
              try
                len := ap_get_client_block(FRequest_rec, buf, size);
                buf[len] := #0;
                FContent := buf;
              finally
                FreeMem(buf, size + 1);
...
begin
  ...
  begin
    ...
    for I := 0 to Cookies.Count - 1 do
      AddHeaderItem('Set-Cookie', Cookies[I].HeaderValue); {do not localize}

    AddHeaderItem('Derived-From', DerivedFrom); {do not localize}

    if Expires > 0 then
      AddHeaderItem('Expires', Format(FormatDateTime(sDateFormat + ' "GMT"'#13#10, Expires), {do not localize}
        [DayOfWeekStr(Expires), MonthStr(Expires)]));

    if LastModified > 0 then
      AddHeaderItem('Last-Modified', Format(FormatDateTime(sDateFormat + ' "GMT"'#13#10,LastModified), {do not localize}
        [DayOfWeekStr(LastModified), MonthStr(LastModified)]));
