  ...
  begin
    ...
    try
      for I := 0 to ControlCount - 1 do
        ...
        begin
          Top := Top + Y;//VertScrollBar.Position;
          Left := Left + X; //HorzScrollBar.Position;
        end;
...
var
  Temp: Integer;
begin
  if (FormStyle <> fsMDIChild) and ((Menu <> nil) and not Menu.AutoMerge) or (csDesigning in ComponentState) then
  begin
    if Menu = nil then
      Temp := 0
    else
...
begin
  Result := False;
  if Assigned(FOnShortCut) then FOnShortCut(Key, Shift, Result);
  Result := Result or (MainForm <> nil) and MainForm.Enabled and
    MainForm.IsShortCut(Key, Shift, KeyText)
end;
