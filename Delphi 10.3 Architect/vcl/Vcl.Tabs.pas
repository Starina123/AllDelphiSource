        ...
        begin
          ...
          begin
            if seFont in StyleElements then
              FMemBitmap.Canvas.Font.Color := StyleServices.GetSystemColor(clBtnText)
            else
             FMemBitmap.Canvas.Font.Color := Self.Font.Color;
          end
          else
          if GetThemeColor(StyleServices.GetElementDetails(TabState[isSelected]), ecTextColor, LColor)
          then
...
begin
  ...
  begin
    Brush.Style := bsSolid;
    if not (not (seClient in StyleElements) and (TStyleManager.IsCustomStyleActive))
        and GetThemeColor(StyleServices.GetElementDetails(tbsBackground), ecFillColor, LColor) then
      Brush.Color := LColor
    else
