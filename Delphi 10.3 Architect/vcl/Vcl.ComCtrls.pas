...
{$ENDIF}
  ...
  Winapi.UxTheme, Winapi.DwmApi, System.StrUtils, Winapi.CommDlg, System.Math, System.DateUtils;
...
begin
  ...
  if not (csDesigning in ComponentState) and
     not (csReading in ComponentState) and HandleAllocated then
    ReAlign;
end;
...
procedure TCustomTreeView.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
        ...
        begin
          ...
          if RightClickSelect then
            ...
            begin
              ...
              Perform(WM_CONTEXTMENU, Handle, MakeLParam(MousePos.X, MousePos.Y));
              FRClickNode := nil;
            end
          else
            // Win95/98 eat WM_CONTEXTMENU when posted to the message queue
            with MousePos do
              PostMessage(Handle, CN_BASE + WM_CONTEXTMENU, Handle, MakeLParam(X, Y));
...
begin
  ...
  begin
    FReadOnly := Value;
    SetComCtlStyle(Self, LVS_EDITLABELS, not Value);
  end;
end;
...
begin
  ...
  begin
    Pos := Index;
    Reordered := not IScaling and (FToolBar.ReorderButton(Pos, ALeft, ATop) <> Pos);
    if Reordered then
    begin
      NeedsUpdate := False;
...
begin
  if HandleAllocated and (Sender = Images) and
     not TCustomImageList(Sender).Scaling then
    RecreateButtons;
end;
...
begin
  if HandleAllocated and (Sender = DisabledImages) and
    not TCustomImageList(Sender).Scaling then RecreateButtons;
end;
...
begin
  if HandleAllocated and (Sender = HotImages) and
    not TCustomImageList(Sender).Scaling then RecreateButtons;
end;
  ...
  if not isDPIChange then
    for I := 0 to ControlCount - 1 do
      if not (Controls[I] is TToolButton) then
        TCtrlClass(Controls[I]).ChangeScale(M, D);
...
procedure TPageScroller.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
  ButtonSize := MulDiv(ButtonSize, M, D);
  if FControl <> nil then
    UpdatePreferredSize;
end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              MCWidth := 1;
              for I := 1 to 7 do
                MCWidth := Max(MCWidth, Length(FormatSettings.ShortDayNames[I]));
              if MCWidth = 1 then
                MCWidth := 3;
              Buffer := TBitmap.Create;
              try
