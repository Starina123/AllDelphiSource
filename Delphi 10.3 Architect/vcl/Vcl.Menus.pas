...
{$ENDIF}
  ...
  begin
    Height := 0;
    Width := 0;
    SPPI := Screen.PixelsPerInch;
    with TStyleManager.SystemStyle do
    begin
      // Calculate check/glyph size
      GetElementSize(ACanvas.Handle, GetElementDetails(CheckMarkStates[Enabled, RadioItem]),
        esActual, GlyphSize);

      GlyphSize.cy := MulDiv(GlyphSize.cy, LPPI, SPPI);
      GlyphSize.cx := MulDiv(GlyphSize.cx, LPPI, SPPI);
      ACanvas.Font.Size := MulDiv(ACanvas.Font.Size, LPPI, SPPI);
      ...
      begin
        ...
        GetElementSize(ACanvas.Handle, GetElementDetails(tmMenuBarItemNormal), esActual, LSize, LPPI);
        Inc(Height, LSize.cy);
      end
      else
      begin
        ...
        GetElementMargins(ACanvas.Handle, GetElementDetails(CheckMarkStates[Enabled, RadioItem]),
          emContent, LMargins, LPPI);
        ...
        GetElementSize(ACanvas.Handle, GetElementDetails(tmPopupGutter), esActual, LSize);
        Inc(Width, LSize.cx);
        GetElementMargins(ACanvas.Handle, GetElementDetails(tmPopupGutter), emSizing, LMargins);
        Inc(Width, LMargins.cxLeftWidth + LMargins.cxRightWidth);

        // Add margins for menu item
        GetElementMargins(ACanvas.Handle, GetElementDetails(tmPopupItemNormal), emSizing, LMargins);
        Inc(Width, LMargins.cxLeftWidth + LMargins.cxRightWidth);

        // Add size of potential submenu glyph
        GetElementSize(ACanvas.Handle, GetElementDetails(tmPopupSubMenuNormal), esActual, LSize, LPPI);
        Inc(Width, LSize.cx);

        // Adjust size if separator
        if Caption = cLineCaption then
        begin
          GetElementSize(ACanvas.Handle, GetElementDetails(tmPopupSeparator), esActual, LSize);
          GetElementMargins(ACanvas.Handle, GetElementDetails(tmPopupSeparator), emSizing, LMargins);
