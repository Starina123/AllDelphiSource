...
begin
  ...
  if TStyleManager.IsCustomStyleActive and (shDialogs in TStyleManager.SystemHooks)
  then
...
begin
  if (NewStyleControls and not (ofOldStyleDialog in Options) and
      not ((Win32MajorVersion >= 6) and UseLatestCommonDialogs)) or
     (TStyleManager.IsCustomStyleActive and (shDialogs in TStyleManager.SystemHooks))
  then
    Template := 'DLGTEMPLATE'
  else
    Template := nil;
...
begin
  ...
  if (TStyleManager.IsCustomStyleActive and 
     not (shDialogs in TStyleManager.SystemHooks))
  then 
