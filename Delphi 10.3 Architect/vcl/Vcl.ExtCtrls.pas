...
begin
  ...
  begin
    ...
    begin
      LHeight := Round(16 * FCurrentPPI / 96);
      LWidth := Round(16 * FCurrentPPI / 96);
    end;
  end;
...
begin
  ...
  if (FPanelGroup.HeaderStyle = hsThemed) and LStyle.Enabled then
    LStyle.DrawElement(ACanvas.Handle,
      LStyle.GetElementDetails(ThemedChevron[FHeaderState, FCollapsed]), FGlyphRect, nil, FCurrentPPI)
  else
  begin
    if UsingImageList then
    begin
    ...
    begin
      // draw chevron
      LChevronSize := Round(6 * FCurrentPPI / 96);
...
begin
  ...
  StyleServices.DrawParentBackground(Handle, TWMCtlColorStatic(Message).ChildDC, nil, False);
  SetTextColor(TWMCtlColorStatic(Message).ChildDC, StyleServices.GetSystemColor(clBtnText));
  Message.Result := GetStockObject(NULL_BRUSH);
  Handled := True;
end;
