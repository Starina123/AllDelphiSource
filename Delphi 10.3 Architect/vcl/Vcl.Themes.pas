...
begin
  Result := DrawText(DC, Details, S, R, TTextFormatFlags(Flags), DPI);
end;
...
begin
  ...
  if AClassName = ToolTipsClassName then
    ...
    Result := shDialogs in TStyleManager.SystemHooks;
end;
...
begin
  ...
  begin
    ...
    if not FIsVCLControl and FRegSysStylesList.ContainsKey(sClassName) and TStyleManager.CheckSysClassName(sClassName) then
    begin
      if Parent > 0 then
