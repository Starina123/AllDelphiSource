...
type
  ...
  TBackendAuthApiHelper = class helper for TBackendAuthApi
  public
  ...
  TLogoutProc = reference to procedure(AServiceAPI: TObject);

procedure RegisterLogoutProc(const AServiceAPIClass: TClass; AProc: TLogoutProc);
procedure CallLogoutProc(const ABackendAuthApi: IBackendAuthApi);
...
var
  FLogoutProcDict: TDictionary<TClass, TLogoutProc> = nil;

procedure RegisterLogoutProc(const AServiceAPIClass: TClass; AProc: TLogoutProc);
begin
  if FLogoutProcDict = nil then
    FLogoutProcDict := TDictionary<TClass, TLogoutProc>.Create;
  FLogoutProcDict.AddOrSetValue(AServiceAPIClass, AProc);
end;
...
begin
  if FLogoutProcDict = nil then
    Exit;
  LApi := ABackendAuthApi as TObject;
  if FLogoutProcDict.TryGetValue(LApi.ClassType, LProc) then
    LProc(LApi);
end;
...
procedure TBackendAuthApiHelper.LogoutUser;
begin
  CallLogoutProc(GetServiceApi);
