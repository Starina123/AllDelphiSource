    ...
    try
      ...
      begin
        ...
        if FDBCtrlGrid.FPanelBorder = gbRaised then
          InflateRect(R, -2, -2);
        if TStyleManager.IsCustomStyleActive and (seClient in FDBCtrlGrid.StyleElements) then
        begin
          FDBCtrlGrid.FCanvas.Brush.Color := StyleServices.GetSystemColor(clBtnFace);
          FDBCtrlGrid.FCanvas.Font.Color := StyleServices.GetSystemColor(clBtnText);
...
begin
  if Erase then
  begin
    if Selected and not (TStyleManager.IsCustomStyleActive and (seClient in StyleElements)) then
    ...
    FillRect(DC, R, Brush);
    if TStyleManager.IsCustomStyleActive and (seClient in StyleElements) and Selected then
    begin
