...
{$ENDIF}
  ...
  private
    function Getcrypt_state: Integer;
  public
    // FB30x
                                                                    
                                                           
    property crypt_state: Integer read Getcrypt_state;
  end;
...
function IsWarning(AError: TIBError): Boolean; inline;
begin
  Result := (AError.FStatusVector[2] = isc_arg_warning) and
    (AError.FStatusVector[1] = 0) and
    (AError.FStatusVector[0] = isc_arg_gds);
end;

{-------------------------------------------------------------------------------}
procedure TIBObject.Check(AStatus: ISC_STATUS; AInitiator: TObject);
begin
  if (AStatus = 0) and not IsWarning(Error) then
    Exit;
...
begin
  if (AStatus = 0) and IsWarning(Self) then begin
...
{ TIBDatabaseHelper                                                             }
{-------------------------------------------------------------------------------}
function TIBDatabaseHelper.Getcrypt_state: Integer;
begin
  Result := GetDatabaseInfoInt(fb_info_crypt_state);
end;
