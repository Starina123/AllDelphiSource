...
begin
  ...
  if ReadSQL <> '' then
    ...
    if BatchMove.Mappings.Count > 0 then begin
      sFields := '';
      for i := 0 to BatchMove.Mappings.Count - 1 do begin
        if i > 0 then
          sFields := sFields + ', ';
        if BatchMove.Mappings[i].DestinationFieldName <> '' then
          sFields := sFields + BatchMove.Mappings[i].DestinationFieldName
        else
          sFields := sFields + BatchMove.Mappings[i].SourceFieldName;
      end;
    end
    else
      sFields := '*';
    FReadQuery.SQL.Text := 'SELECT ' + sFields + ' FROM ' + ActualTableName + ' WHERE 0=1';
  end;
  try
    FReadQuery.Open;
  except
