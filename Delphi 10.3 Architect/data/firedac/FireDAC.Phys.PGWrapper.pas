...
{$IFDEF FireDAC_PGSQL_STATIC}
 ...
 {$ELSE}
  ...
  FPQencryptPassword := GetProc(sPQencryptPassword);
  ...
  try
    ...
    if iBase <> 0 then
      ...
      if FConnection.UnknownFormat = 0 then
        DescribeError(sName, 'A type must be an array, composite, enum, range or base type')
      else
    ...
    // base type
      ...
      if sCat = 'B' then
        ...
        DescribeError(sName, 'A type is not a simple base type')
      else
...
begin
  ...
  FServerVersion := 0;
  try
    sConnBytes := Encoder.Encode(AConnStr, ecUTF8);
