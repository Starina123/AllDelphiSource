...
{$ENDIF}
  ...
  private const
    CKeyEncoding: TFDEncoding = {$IFDEF FireDAC_SQLITE_STATIC} ecUTF8 {$ELSE} ecANSI {$ENDIF};
  private
    FBuffer: TFDBuffer;
    FEncoder: TFDEncoder;
...
begin
  if Assigned(Lib.Fsqlite3_key) then begin
    sKey := Encoder.Encode(AKey, CKeyEncoding);
{$IFDEF FireDAC_MONITOR}
    if Tracing then Trace1;
{$ENDIF}
    Check(Lib.Fsqlite3_key(Handle, PByte(sKey), Encoder.EncodedLength(sKey, CKeyEncoding)));
  end
  else
...
{$IFDEF FireDAC_MONITOR}
    if Tracing then Trace1;
{$ENDIF}
    Check(Lib.Fsqlite3_rekey(Handle, PByte(sKey), Encoder.EncodedLength(sKey, CKeyEncoding)));
  end
  else
    FDCapabilityNotSupported(nil, [S_FD_LPhys, S_FD_SQLiteId, 'sqlite3']);
end;
