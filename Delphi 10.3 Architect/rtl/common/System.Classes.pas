...
{$IF Defined(POSIX)}
  ...
  Posix.SysSelect, Posix.SysTime, Posix.Time, Posix.Semaphore,
{$IF Defined(LINUX)}
  Posix.Fcntl,
...
type
  ...
  public
    ...
    class property Global: TFieldsCache read FGlobal;
  end;
...
begin
  ...
  begin
    ...
    if FCompareStringsOverriden then
...
begin
  if not FCompareStringsOverriden then
...
begin
  ...
  FCompareStringsOverriden := TMethod(CompareStringsP).Code <> @TStringList.CompareStrings;
end;
...
{ TReader }
procedure WriterWriteUTF8Str(Writer: TWriter; const Value: string); forward;
...
begin
  ...
  if UseQualifiedNames then
    WriterWriteUTF8Str(Self, Instance.ClassType.UnitName + '.' + Instance.ClassName)
  else
    WriterWriteUTF8Str(Self, Instance.ClassName);
  WriterWriteUTF8Str(Self, Instance.Name);
  ...
  begin
    ...
    for I := 0 to SizeOf(TIntegerSet) * 8 - 1 do
      if I in TIntegerSet(Value) then
        WriterWriteUTF8Str(Self, GetSetElementName(BaseType, I));
    WriterWriteUTF8Str(Self, '');
  end;
...
begin
  WriterWriteUTF8Str(Self, FPropPath + PropName);
end;
...
procedure WriterWriteUTF8Str(Writer: TWriter; const Value: string);
var
...
procedure ParserError(AParser: TParser; const IdentRes: PResStringRec);
begin
  AParser.ErrorStr(LoadResString(IdentRes));
end;

procedure ParserErrorFmt(AParser: TParser; const IdentRes: PResStringRec; const Args: array of const);
begin
  AParser.ErrorStr(Format(LoadResString(IdentRes), Args));
end;
...
begin
  if Token <> T then
    case T of
      toSymbol:
        ParserError(Self, @SIdentifierExpected);
      System.Classes.toString, toWString:
        ParserError(Self, @SStringExpected);
      toInteger, toFloat:
        ParserError(Self, @SNumberExpected);
    else
      ParserErrorFmt(Self, @SCharExpected, [T]);
    end;
end;
...
begin
  if not TokenSymbolIs(S) then
    ParserErrorFmt(Self, @SSymbolExpected, [S]);
end;
...
begin
  ...
  begin
    ...
    if Count = 0 then
    begin
      ParserError(Self, @SInvalidBinary);
      Exit;
    end;
              ...
              begin
                ...
                begin
                  case FBuffer[P] of
                    0, 10, 13:
                      begin
                        ParserError(Self, @SInvalidString);
                        Break;
                      end;
                    Ord(''''):
                      begin
...
var
  ...
  begin
    ...
    WriterWriteUTF8Str(Writer, ClassName);
    WriterWriteUTF8Str(Writer, ObjectName);
  end;
  ...
  begin
    ...
    begin
      case Parser.Token of
          ...
          begin
            ...
            if Parser.Token <> ']' then
              ...
              begin
                ...
                WriterWriteUTF8Str(Writer, TokenStr);
                if Parser.NextToken = ']' then Break;
                Parser.CheckToken(',');
                Parser.NextToken;
              end;
            WriterWriteUTF8Str(Writer, '');
          end;
        '(':
          begin
            Parser.NextToken;
        ...
        ParserError(Parser, @SInvalidProperty);
      end;
      Parser.NextToken;
    end;
  end;
  ...
  begin
    ...
    WriterWriteUTF8Str(Writer, PropName);
    Parser.CheckToken('=');
    Parser.NextToken;
    ConvertValue;
  end;
...
{$IFDEF POSIX}
  ...
  ResumeEvents: TDictionary<TThread, Psem_t> = nil;
{$ENDIF}
...
begin
  ...
  begin
    ...
    if ResumeEvents <> nil then
    begin
      TMonitor.Enter(ResumeEvents);
      try
        New(ResumeEvent);
        sem_init(ResumeEvent^, 0, 0);
        ResumeEvents.Add(Self, ResumeEvent);
      finally
        TMonitor.Exit(ResumeEvents);
      end;
    end;
...
{$IFDEF POSIX}
  ResumeEvents := TDictionary<TThread, Psem_t>.Create;
{$ENDIF}
end;
...
{$ELSEIF Defined(POSIX)}
  ...
  begin
    ...
    begin
      TMonitor.Enter(ResumeEvents);
      try
        if ResumeEvents.TryGetValue(Self, ResumeEvent) then
        begin
          ResumeEvents.Remove(Self);
          sem_destroy(ResumeEvent^);
          Dispose(ResumeEvent);
        end;
      finally
        TMonitor.Exit(ResumeEvents);
  ...
  But when Suspend is called from the thread Execute, it will wait for event set in Resume.
    ...
    if GetCurrentThreadId = ThreadId then
      ...
      begin
        TMonitor.Enter(ResumeEvents);
        try
          ResumeEventFound := ResumeEvents.TryGetValue(Self, ResumeEvent);
        finally
          TMonitor.Exit(ResumeEvents);
        end;
        if ResumeEventFound then
          sem_wait(ResumeEvent^);
      end;
{$ENDIF POSIX}
  except
...
begin
  ...
  begin
    ...
    begin
      ...
      if ResumeEventFound then
      begin
        sem_getvalue(ResumeEvent^, I);
        if I = 0 then
          sem_post(ResumeEvent^);
      end;
    end;
  FSuspended := False;
end;
