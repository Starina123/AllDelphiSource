...
function TListHelper.DoIndexOfFwd1(const Value): Integer;
  ...
  function DoIndexOfFwdUsingComparer(const Value): Integer;
  var
    P: PByte;
  begin
    P := Pointer(FItems);
    ...
    begin
      if FCompare(FListObj, P^, Byte(Value)) = 0 then
        Exit;
      Inc(P);
    end;
    Result := -1;
...
begin
  if Assigned(FCompare) then
    Exit(DoIndexOfFwdUsingComparer(Value));
  P := Pointer(FItems);
  for Result := 0 to FCount - 1 do
  begin
...
function TListHelper.DoIndexOfFwd2(const Value): Integer;
  ...
  begin
    ...
    for Result := 0 to FCount - 1 do
    begin
      if FCompare(FListObj, P^, Word(Value)) = 0 then
        Exit;
      Inc(P);
    end;
    Result := -1;
...
function TListHelper.DoIndexOfFwd4(const Value): Integer;
  ...
  begin
    ...
    for Result := 0 to FCount - 1 do
    begin
      if FCompare(FListObj, P^, Cardinal(Value)) = 0 then
        Exit;
      Inc(P);
    end;
    Result := -1;
...
function TListHelper.DoIndexOfFwd8(const Value): Integer;
  ...
  begin
    ...
    for Result := 0 to FCount - 1 do
    begin
      if FCompare(FListObj, P^, UInt64(Value)) = 0 then
        Exit;
      Inc(P);
    end;
    Result := -1;
...
function TListHelper.DoIndexOfFwdMRef(const Value): Integer;
  ...
  begin
    ...
    for Result := 0 to FCount - 1 do
    begin
      if FCompare(FListObj, P^, Pointer(Value)) = 0 then
        Exit;
      Inc(P);
    end;
    Result := -1;
...
function TListHelper.DoIndexOfFwdString(const Value): Integer;
  ...
  begin
    ...
    for Result := 0 to FCount - 1 do
    begin
      if FCompare(FListObj, P^, UnicodeString(Value)) = 0 then
        Exit;
      Inc(P);
    end;
    Result := -1;
...
function TListHelper.DoIndexOfRev1(const Value): Integer;
  ...
  function DoIndexOfRevUsingComparer(const Value): Integer;
  var
    P: PByte;
  begin
    P := PByte(Pointer(FItems)) + FCount - 1;
...
begin
  if Assigned(FCompare) then
    Exit(DoIndexOfRevUsingComparer(Value));
  P := PByte(Pointer(FItems)) + FCount - 1;
  for Result := FCount - 1 downto 0 do
  begin
    if P^ = Byte(Value) then
...
function TListHelper.DoIndexOfRev2(const Value): Integer;
  ...
  var
    P: PWord;
  begin
    P := PWord(Pointer(FItems)) + FCount - 1;
    for Result := FCount - 1 downto 0 do
    begin
      if FCompare(FListObj, P^, Word(Value)) = 0 then
        Exit;
...
function TListHelper.DoIndexOfRev4(const Value): Integer;
  ...
  var
    P: PCardinal;
  begin
    P := PCardinal(Pointer(FItems)) + FCount - 1;
    for Result := FCount - 1 downto 0 do
    begin
      if FCompare(FListObj, P^, Cardinal(Value)) = 0 then
        Exit;
...
function TListHelper.DoIndexOfRev8(const Value): Integer;
  ...
  var
    P: PUInt64;
  begin
    P := PUInt64(Pointer(FItems)) + FCount - 1;
    for Result := FCount - 1 downto 0 do
    begin
      if FCompare(FListObj, P^, UInt64(Value)) = 0 then
        Exit;
...
function TListHelper.DoIndexOfRevMRef(const Value): Integer;
  ...
  var
    P: PPointer;
  begin
    P := PPointer(Pointer(FItems)) + FCount - 1;
    for Result := FCount - 1 downto 0 do
    begin
      if FCompare(FListObj, P^, Pointer(Value)) = 0 then
        Exit;
...
function TListHelper.DoIndexOfRevString(const Value): Integer;
  ...
  var
    P: PUnicodeString;
  begin
    P := PUnicodeString(Pointer(FItems)) + FCount - 1;
    for Result := FCount - 1 downto 0 do
    begin
      if FCompare(FListObj, P^, UnicodeString(Value)) = 0 then
        Exit;
...
begin
  Result := arrayofT(FListHelper.FItems);
end;
...
begin
  Result := TArray.BinarySearch<T>(List, Item, Index, FComparer, 0, Count);
end;

function TList<T>.BinarySearch(const Item: T; out Index: Integer;
  const AComparer: IComparer<T>): Boolean;
begin
  Result := TArray.BinarySearch<T>(List, Item, Index, AComparer, 0, Count);
end;
...
{$ENDIF}
    ...
    FListHelper.FCompare := @TList<T>.InternalCompare
  else
    FListHelper.FCompare := nil;
end;
...
begin
  ...
  if LAssign then
    FListHelper.FNotify := @TList<T>.InternalNotify
  else
    FListHelper.FNotify := nil;
end;
...
begin
  if FListHelper.FCount = Length(List) then
    GrowCheck(FCount + 1);
  Result := Self;
end;
...
begin
  if FLH.FCount = DynArraySize(FItems) then
    InternalGrowMRef;
  TListHelper.PBytes(FItems)[FHead] := TBytes(Value);
  EnqueueAdjust(Value);
end;
...
begin
  inherited Create;
  FQueueHelper.FLH.FListObj := Self;
  FQueueHelper.FLH.FNotify := @TQueue<T>.InternalNotify;
  FQueueHelper.FLH.FCompare := @TQueue<T>.InternalCompare;
...
begin
  Result := arrayOfT(FQueueHelper.FItems);
end;
...
begin
  Result := FQueue.List[(FQueue.FQueueHelper.FTail + FIndex) mod Length(FQueue.List)];
end;
...
begin
  inherited Create;
  FStackHelper.FLH.FListObj := Self;
  FStackHelper.FLH.FNotify := @TStack<T>.InternalNotify;
...
begin
  Result := arrayOfT(FStackHelper.FLH.FItems);
end;
