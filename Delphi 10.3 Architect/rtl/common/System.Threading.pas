...
type
  ...
  protected type
    TOptionStateFlag = (Started, CallbackRun, ChildWait, Complete, Canceled, Faulted, Replicating, Replica, Raised, ForceSize = SizeOf(Integer) * 8 - 1);
...
begin
  Result := TJoinTask.Create(AProcs, TThreadPool.Current).Start;
end;
...
begin
  Result := TJoinTask.Create(Sender, AEvents, TThreadPool.Current).Start;
end;
  ...
  ReplicaProc := procedure
    ...
    begin
      ...
      if not Root.ShouldCreateReplica {or ReplicasQuitting} then
        Exit;
      ChildTask := Root.CreateReplicaTask(ReplicaProc, Root, [TCreateFlag.Replicating, TCreateFlag.Replica]);
      ChildTask.Start;
      try
        Root.CallUserCode;
      except
...
class function TThreadPoolStats.GetCurrentThreadPoolStats: TThreadPoolStats;
begin
  Result := GetThreadPoolStats(TThreadPool.Current);
end;

class function TThreadPoolStats.GetDefaultThreadPoolStats: TThreadPoolStats;
begin
  Result := GetThreadPoolStats(TThreadPool.Default);
end;

class function TThreadPoolStats.GetThreadPoolStats(const Pool: TThreadPool): TThreadPoolStats;
begin
