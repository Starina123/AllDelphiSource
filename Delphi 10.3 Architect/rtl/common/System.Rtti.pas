...
function HasEnumNameList(AType: TRttiEnumerationType): Boolean;
begin
  // No name list if we are a subrange of another enumeration.
  Result := (AType.TypeData^.BaseType = nil) or
    (DerefPointer(AType.TypeData^.BaseType) = AType.Handle) or
    // Special case with built-in Boolean enum
    IsBoolType(AType.Handle);
end;
...
{$ELSEIF Defined(MACOS64) or Defined(LINUX64)}
          if (parList[i-1].ParamType <> nil) and (
               ((parList[i - 1].ParamType.TypeKind in [tkRecord, tkMRecord, tkArray]) and
                (ii.Method.CallingConvention in [ccReg, ccPASCAL]) and
                (parList[i - 1].ParamType.TypeSize > 4))
            or ([pfVar, pfOut] * parList[i - 1].Flags <> [])) then
{$ELSEIF defined(CPUARM)}
          if (parList[i-1].ParamType <> nil) and (
