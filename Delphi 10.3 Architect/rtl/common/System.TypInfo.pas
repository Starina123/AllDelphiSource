...
{$ENDIF POSIX}
  System.Rtti, System.RTLConsts, System.SysConst;
...
begin
  ...
  begin
    ...
    if SameText(string(LStr), Name) then
      Exit(I);
    Inc(LName, LName^ + 1);
  end;
...
begin
  if SizeOfSet(PropInfo^.PropType^) <= SizeOf(TReturnOnRegisterSet) then
    LargestSet := TPropSet<TReturnOnRegisterSet>.GetProc(Instance, PropInfo)
  else
    LargestSet := TPropSet<TLargestSet>.GetProc(Instance, PropInfo);
...
begin
  ...
  begin
    ...
    if (Info^.Kind = tkMRecord) and (numOps > 4) and ((PNativeUInt(RecOps(p, 4))^ and 1) <> 0) then
      Result := rmByValueStack
    else if (Info^.Kind = tkMRecord) and
            ( ((NumOps > 0) and (RecOps(p, 0) <> nil)) or  // roiInitializer
              ((NumOps > 1) and (RecOps(p, 1) <> nil)) or  // roiFinalizer
              ((NumOps > 3) and (RecOps(p, 3) <> nil)) ) then // roiCopyConstructor
      Result := rmByRef
    else
      Result := rmByValueReg;
  end
