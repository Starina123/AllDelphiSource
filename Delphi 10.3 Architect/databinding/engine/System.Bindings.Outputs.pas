...
{$ENDIF}
  ...
  TValueRefConverterFactory.RegisterConversion(TypeInfo(TPersistent), TypeInfo(string),
...
begin
  O := TValue.From<string>('(OBJECT)')  // Do not localize
end;
    ...
    case ATo.Kind of
      tkInteger:
        case ATo.TypeData.OrdType of
          ...
          otUWord,
          otULong:
            O := TValue.From<Uint32>(StrToUInt64(val))
        end;
      tkInt64:
        with I.TypeInfo.TypeData^ do
          if MinInt64Value > MaxInt64Value then
            O := TValue.From<uint64>(StrToUInt64(val))
          else
...
begin
  if TryStrToDateTIme(I.AsString, LDateTime) then
    O := TValue.From<TDateTime>(LDateTime)
  else
    O := TValue.Empty;
end;
  ...
  case ATo.Kind of
    tkInteger:
      case ATo.TypeData.OrdType of
        ...
        otUWord,
        otULong:
          O := TValue.From<Uint32>(Round(I.AsExtended))
      end;
    tkInt64:
      with I.TypeInfo.TypeData^ do
        if MinInt64Value > MaxInt64Value then
          O := TValue.From<uint64>(Round(I.AsExtended))
        else
          O := TValue.From<int64>(Round(I.AsExtended));
  else
    Assert(False);
    O := TValue.Empty;
  end;
