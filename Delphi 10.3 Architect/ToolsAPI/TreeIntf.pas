...
var
  LService: IOTAStructureView;
  LPalServices: IOTAPaletteServices;
  LGroupClass: TClass;
  LIDString: string;
  LBitmap: TBitmap;
  LPainter: INTAPalettePaintIcon;
...
begin
  ...
  begin
    ...
    begin
      if Supports(BorlandIDEServices, IOTAStructureView, LService) and
      ...
      begin
        ...
        if Supports(LPaletteItem, INTAPalettePaintIcon, LPainter) then
        begin
          LBitmap := TBitmap.Create;
          try
            LBitmap.SetSize(16, 16);
            LPainter.Paint(LBitmap.Canvas, 0, 0, pi16x16);
            LBitmap.PixelFormat := TPixelFormat.pf24bit;
            FIconIndex := LService.AddBitmap(LBitmap.Handle);
...
begin
  if Supports(BorlandIDEServices, IOTAServices, LServices) and
    Supports(BorlandIDEServices, IOTAStructureView, LStructureView) then
  begin
    LEmbedded := LServices.GetEnvironmentOptions.GetOptionValue('InternalEmbeddedDesigner');
    if not LEmbedded then
