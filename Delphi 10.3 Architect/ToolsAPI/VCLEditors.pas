...
procedure DefaultPropertyDrawName(Prop: TPropertyEditor; Canvas: TCanvas;
  const Rect: TRect; Spacing : Integer = 0);
begin
  Canvas.TextRect(Rect, Rect.Left + 2 + Spacing, Rect.Top + 1, Prop.GetName);
end;

procedure DefaultPropertyDrawValue(Prop: TPropertyEditor; Canvas: TCanvas;
  const Rect: TRect; Spacing : Integer = 0);
begin
  Canvas.TextRect(Rect, Rect.Left + 1 + Spacing, Rect.Top + 1, Prop.GetVisualValue);
end;

procedure DefaultPropertyListDrawValue(const Value: string; Canvas: TCanvas;
  const Rect: TRect; Selected: Boolean; Spacing : Integer = 0);
begin
  Canvas.TextRect(Rect, Rect.Left + 1 + Spacing, Rect.Top + 1, Value);
end;
...
begin
  ...
  begin
    ...
    if CursorHandle <> 0 then
      DrawIconEx(ACanvas.Handle, ARect.Left + 2 + PropertyDrawingOffset, 
        ARect.Top + 2, CursorHandle, 0, 0, 0, 0, DI_NORMAL or DI_DEFAULTSIZE);
    DefaultPropertyListDrawValue(Value, ACanvas, Rect(Right, ARect.Top,
  ...
  begin
    ...
    try
      LStyle.DrawElement(ACanvas.Handle,
        LStyle.GetElementDetails(ThemeStyles[State][AEnabled]), ARect);
    finally
