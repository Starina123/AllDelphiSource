...
uses
  ...
  FmxStrEdit,
  {$ENDIF}
  FMX.Editor.Items, FmxSprigs, FmxStyleLookup, FMX.Types, FMX.Design.Font, FMX.Canvas.D2D, Winapi.D2D1;
...
begin
  ...
  try
    ...
    EditDlg.PathData.OnChangeTracking := PathDataChangeTracking;
    if EditDlg.ShowModal = mrOk then
    begin
...
procedure TPathDataProperty.PathDataChangeTracking(Sender: TObject);
var
  Form: TPathDataDesigner;
begin
  if Sender is TControl then
  begin
    Form := TControl(Sender).Root.GetObject as TPathDataDesigner;
    if Form = nil then
      Exit;

    try
      if Form.PreviewLayout.Visible then
        Form.PreviewPath.Data.Data := Form.PathData.Text;
